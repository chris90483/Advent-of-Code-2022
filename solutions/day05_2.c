...
void execute_instruction(struct Instruction *instruction, struct Stack **stacks) {
    // contents that will be moved, starting with the content that should appear on top.
    char contents_to_move[instruction->amount_to_move];
    
    struct Stack *to_move = stacks[instruction->source_stack - 1];
    for (size_t i = 0; i < instruction->amount_to_move; i++) {
        contents_to_move[i] = to_move->top_crate_content;
        to_move = to_move->rest_of_the_stack;
    }        
         
    // create updated destination stack, from the bottom up
    struct Stack *destination_stack = stacks[instruction->destination_stack - 1];
    for (size_t i = 0; i < instruction->amount_to_move; i++) {
        struct Stack *updated_destination_stack = malloc(sizeof(struct Stack));
        updated_destination_stack->top_crate_content = contents_to_move[instruction->amount_to_move - (i + 1)];
        updated_destination_stack->rest_of_the_stack = destination_stack;
        destination_stack = updated_destination_stack;
    }

    // put updated stack on destination
    stacks[instruction->destination_stack - 1] = destination_stack;

    // remove contents from source, free the memory
    struct Stack *source_stack = stacks[instruction->source_stack - 1];
    for(size_t i = 0; i < instruction->amount_to_move; i++) {
        struct Stack *to_remove = source_stack;
        source_stack = to_remove->rest_of_the_stack;
        free(to_remove);
    }
    stacks[instruction->source_stack - 1] = source_stack;
}
